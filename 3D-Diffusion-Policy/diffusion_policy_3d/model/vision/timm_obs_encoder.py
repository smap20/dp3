...
class TimmObsEncoder(ModuleAttrMixin):
    def __init__(self,
        ...
        if model_name.startswith('dinov2'):
            ...
            self.timm_transforms = None
            # self.timm_transforms = processor
            # model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14')
            # model = torch.load(os.path.expanduser("~/.cache/torch/hub/checkpoints/dinov2_vits14_pretrain.pth"))
            # import ipdb; ipdb.set_trace()
            # model = torch.hub.load('facebookresearch/dinov2', '/home/zhengrong/projects/Spatial-Generalization/data/cached_models/dinov2_vits14_pretrain.pth')
        # elif model_name.startswith('theia'):
        #     full_name = f"theaiinstitute/{model_name}-patch16-224-cdiv"
        ...
        #     model = AutoModel.from_pretrained("theaiinstitute/theia-base-patch16-224-cdiv", trust_remote_code=True)
            ...
            print("timm_transform:", self.timm_transforms)
        ...
        if model_name.startswith('vit') or model_name.startswith('dino'):
        ...
        if self.feature_aggregation == 'soft_attention':
            ...
            self.attention_pool_2d = AttentionPool2d(
                spacial_dim=7, # hard code for now, will be modify in future
    ...
    def forward(self, obs_dict):
        ...
        for key in self.rgb_keys:
            ...
            if self.timm_transforms is None:
                # img = self.dino_processor(img * 255.)
                # cprint(f"image shape:{img.shape}", 'red')
                raw_feature = self.key_model_map[key](img)["pooler_output"]
                # cprint(f"raw feature shape:{raw_feature.shape}", 'red')
            else:
                img = self.timm_transforms(img)
                raw_feature = self.key_model_map[key](img)

            # cprint(f"image shape:{img.shape}", 'red')
            # if self.timm_transforms is not None:
            #     img = self.timm_transforms(img)
            # cprint(f"image shape transformed:{img.shape}", 'red')

            # raw_feature = self.key_model_map[key].forward_feature(img)
            # raw_feature = self.key_model_map[key](img)["pooler_output"]
